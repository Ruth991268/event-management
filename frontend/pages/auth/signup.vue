    ...
    <div class="w-full max-w-md bg-white rounded-xl shadow-md p-8">
      ...
      <Form @submit="onSubmit" :validation-schema="schema">
        <div class="mb-4">
          ...
          <Field name="name" class="w-full border rounded px-3 py-2" />
      ...
      <div class="text-center mt-4">
        <NuxtLink to="/auth/login" class="text-indigo-600">Already have an account? Login</NuxtLink>
      </div>
    </div>
  </div>
</template>
...
const onSubmit = async (values) => {
  ...
  errorMessage.value = ''
  try {
    const res = await $api('/signup', { method: 'POST', body: values })
    if (res?.token) {
    ...
    errorMessage.value = res?.message || 'Signup failed'
  } catch (err) {
    errorMessage.value = err?.data?.error || err?.message || 'Signup failed'
  } finally {
    loading.value = false
  }
}
